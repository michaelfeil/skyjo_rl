...
class SkyjoGame(object):
    ...
    def observe_player(self, player_id: int):
        ...
        # fill dummy value
        cards = np.full_like(self.players_cards[player_id], self.fill_masked_value)
        # replace thos cards known with the actual value
        cards[self.players_masked[player_id] != 0] = self.players_cards[player_id][
            self.players_masked[player_id] != 0
        ]
    ...
    def observe_global_game_stats(self):
        ...
        for pl in range(self.n_players):
            occured_cards.extend(
                list(self.players_cards[pl][self.players_masked[pl] == 1])
            )
    ...
    def action_place(
        self, player_id: int, picked_card: int, place_to_pos: int
